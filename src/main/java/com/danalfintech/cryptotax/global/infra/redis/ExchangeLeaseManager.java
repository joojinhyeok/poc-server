...
public class ExchangeLeaseManager {
    ...
    public boolean tryAcquire(ExchangeContext ctx, String workerId, int maxConcurrent) {
        try {
            long now = Instant.now().toEpochMilli();
            long expireTime = now + LEASE_TTL_MILLIS;
            Long result = redisTemplate.execute(
                    acquireScript,
                    Collections.singletonList(RedisKeyBuilder.leaseKey(ctx)),
            ...
            log.warn("Redis lease acquire 실패, fail-open 처리: exchange={}, workerId={}", ctx.exchange(), workerId, e);
            return true;
        }
    }

    public void release(ExchangeContext ctx, String workerId) {
        try {
            ...
            log.warn("Redis lease release 실패: exchange={}, workerId={}", ctx.exchange(), workerId, e);
        }
    }

    public void heartbeat(ExchangeContext ctx, String workerId) {
        try {
            long expireTime = Instant.now().toEpochMilli() + LEASE_TTL_MILLIS;
            redisTemplate.opsForHash().put(RedisKeyBuilder.leaseKey(ctx), workerId, String.valueOf(expireTime));
        } catch (Exception e) {
            log.warn("Redis lease heartbeat 실패: exchange={}, workerId={}", ctx.exchange(), workerId, e);
        }
    }
    ...
    public void cleanupExpiredLeases() {
        for (Exchange exchange : Exchange.values()) {
            try {
                ...
                String key = RedisKeyBuilder.leaseKey(ctx);
