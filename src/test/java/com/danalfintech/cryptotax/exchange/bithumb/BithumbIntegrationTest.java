...
@DisplayName("BithumbCollector 통합 테스트")
@Import(BithumbIntegrationTest.MockConnectorConfig.class)
class BithumbIntegrationTest extends BaseIntegrationTest {

    @TestConfiguration
    static class MockConnectorConfig {
        @Bean("bithumbConnector")
        BithumbConnector bithumbConnector() {
            BithumbConnector mock = mock(BithumbConnector.class);
            given(mock.getExchange()).willReturn(Exchange.BITHUMB);
            return mock;
        }
    }
    ...
    void setUp() {
        reset(connector);
        given(connector.getExchange()).willReturn(Exchange.BITHUMB);
    ...
    class CollectSymbol {
        ...
        void multiPageSavesAllTrades() {
            // desc 정렬: page 1에 최신 거래, page 2에 과거 거래
            ...
            given(connector.getTrades(any(), eq("BTC"), isNull(), eq(100)))
                    .willReturn(new TradePageResult(List.of(trade1), true, "2"));
            given(connector.getTrades(any(), eq("BTC"), eq("2"), eq(100)))
        ...
        void createsSyncCursor() {
            ...
            Optional<SyncCursor> cursor = syncCursorRepository
                    .findByUserIdAndExchangeAndSymbol(USER_ID, Exchange.BITHUMB, "ETH");
        ...
        void updatesExistingSyncCursor() {
            ...
            SyncCursor updated = syncCursorRepository
                    .findByUserIdAndExchangeAndSymbol(USER_ID, Exchange.BITHUMB, "BTC").orElseThrow();
            assertThat(updated.getLastSyncedAt())
                    .isEqualTo(LocalDateTime.of(2024, 6, 15, 14, 0, 0));
        }

        @Test
        @DisplayName("증분 수집 시 cutoff 이전 거래는 저장하지 않고 조기 중단한다")
        void incrementalEarlyTermination() {
            // desc 정렬: 새 거래 → 기존 거래
            ...
            given(connector.getTrades(any(), eq("BTC"), isNull(), eq(100)))
                    .willReturn(new TradePageResult(List.of(newTrade, oldTrade), true, "2"));

            SymbolCollectionResult result = collector.collectSymbol(
                    USER_ID, apiKey, "BTC", "2024-06-01T00:00:00");

            assertThat(result.success()).isTrue();
            assertThat(result.newTradesCount()).isEqualTo(1);
            // DB에도 새 거래 1건만 저장
            assertThat(tradeRepository.findAll()).hasSize(1);
            assertThat(tradeRepository.findAll().get(0).getExchangeTradeId()).isEqualTo("uuid-new");
            // page 2는 호출하지 않음 (조기 중단)
            then(connector).should(times(1)).getTrades(any(), eq("BTC"), any(), eq(100));
        }
        ...
        void emptyResponseSavesNothing() {
            ...
            assertThat(syncCursorRepository.findByUserIdAndExchangeAndSymbol(
                    USER_ID, Exchange.BITHUMB, "BTC")).isEmpty();
        }
    ...
    class SyncBalances {
        ...
        void savesNewAsset() {
            ...
            Optional<Asset> asset = assetRepository
                    .findByUserIdAndExchangeAndCoin(USER_ID, Exchange.BITHUMB, "BTC");
        ...
        void updatesExistingAsset() {
            assetRepository.saveAndFlush(Asset.builder()
                    .userId(USER_ID).exchange(Exchange.BITHUMB).coin("ETH")
            ...
            Asset updated = assetRepository
                    .findByUserIdAndExchangeAndCoin(USER_ID, Exchange.BITHUMB, "ETH").orElseThrow();
        ...
        void skipsZeroBalance() {
            ...
            assertThat(assetRepository.findByUserIdAndExchangeAndCoin(
                    USER_ID, Exchange.BITHUMB, "XRP")).isEmpty();
        }
