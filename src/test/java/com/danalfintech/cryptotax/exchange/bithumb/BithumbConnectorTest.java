...
class BithumbConnectorTest {
    ...
    @InjectMocks
    private BithumbConnector connector;
    ...
    void getExchange() {
        assertThat(connector.getExchange()).isEqualTo(Exchange.BITHUMB);
    }
    ...
    class GetBalances {
        ...
        void success() {
            stubRestClientChain();
            BithumbAccount[] accounts = {
                    new BithumbAccount("BTC", "1.5", "0.5", "50000000", false, "KRW"),
                    new BithumbAccount("ETH", "10.0", "0.0", "3000000", false, "KRW")
            };
            given(responseSpec.body(BithumbAccount[].class)).willReturn(accounts);
        ...
        void nullResponse() {
            stubRestClientChain();
            given(responseSpec.body(BithumbAccount[].class)).willReturn(null);
        ...
        void nullAvgBuyPrice() {
            stubRestClientChain();
            BithumbAccount[] accounts = {
                    new BithumbAccount("BTC", "1.0", "0.0", null, false, "KRW")
            };
            given(responseSpec.body(BithumbAccount[].class)).willReturn(accounts);
    ...
    class GetTrades {
        ...
        void success_bidToBuy() {
            stubRestClientChain();
            BithumbTrade[] trades = {
                    new BithumbTrade("uuid-1", "bid", "limit", "50000000",
                            "done", "KRW-BTC", "2024-01-15T10:30:00+09:00",
                            "1.0", "0.0", "1.0", "75000", 1)
            };
            given(responseSpec.body(BithumbTrade[].class)).willReturn(trades);
        ...
        void askToSell() {
            stubRestClientChain();
            BithumbTrade[] trades = {
                    new BithumbTrade("uuid-2", "ask", "limit", "60000000",
        ...
        void filtersZeroExecutedVolume() {
            ...
            BithumbTrade[] trades = {
                    new BithumbTrade("uuid-1", "bid", "limit", "50000000",
                            ...
                            "1.0", "1.0", "0", "0", 0),
                    new BithumbTrade("uuid-2", "bid", "limit", "50000000",
                            "done", "KRW-BTC", "2024-01-15T11:00:00+09:00",
                            "1.0", "0.0", "1.0", "75000", 1)
            };
            given(responseSpec.body(BithumbTrade[].class)).willReturn(trades);
        ...
        @Test
        @DisplayName("응답 건수 == limit이면 hasMore=true, nextCursor는 다음 페이지 번호")
        void hasMorePages() {
            stubRestClientChain();
            BithumbTrade[] trades = {
                    new BithumbTrade("uuid-1", "bid", "limit", "50000000",
                            ...
                            "1.0", "0.0", "1.0", "75000", 1),
                    new BithumbTrade("uuid-2", "bid", "limit", "51000000",
                            "done", "KRW-BTC", "2024-01-15T11:30:45+09:00",
                            "0.5", "0.0", "0.5", "38000", 1)
            };
            given(responseSpec.body(BithumbTrade[].class)).willReturn(trades);

            // fromId=null → page=1, limit=2 → nextCursor="2" (다음 페이지)
        ...
        @Test
        @DisplayName("fromId가 주어지면 해당 페이지부터 조회한다")
        void paginationWithFromId() {
            stubRestClientChain();
            BithumbTrade[] trades = {
                    new BithumbTrade("uuid-3", "bid", "limit", "52000000",
                            "done", "KRW-BTC", "2024-01-15T12:00:00+09:00",
                            "0.3", "0.0", "0.3", "23000", 1),
                    new BithumbTrade("uuid-4", "ask", "limit", "53000000",
                            "done", "KRW-BTC", "2024-01-15T13:00:00+09:00",
                            "0.2", "0.0", "0.2", "16000", 1)
            };
            given(responseSpec.body(BithumbTrade[].class)).willReturn(trades);

            // fromId="3" → page=3, limit=2 → nextCursor="4"
            TradePageResult result = connector.getTrades(apiKey, "BTC", "3", 2);
        ...
        void emptyResponse() {
            stubRestClientChain();
            given(responseSpec.body(BithumbTrade[].class)).willReturn(new BithumbTrade[0]);
        ...
        void nullResponse() {
            stubRestClientChain();
            given(responseSpec.body(BithumbTrade[].class)).willReturn(null);
        ...
        void nullPrice() {
            stubRestClientChain();
            BithumbTrade[] trades = {
                    new BithumbTrade("uuid-1", "bid", "market", null,
                            "done", "KRW-BTC", "2024-01-15T10:30:00+09:00",
                            "1.0", "0.0", "1.0", "75000", 1)
            };
            given(responseSpec.body(BithumbTrade[].class)).willReturn(trades);
        ...
        @Test
        @DisplayName("paidFee가 null인 경우 BigDecimal.ZERO로 매핑한다")
        void nullPaidFee() {
            stubRestClientChain();
            BithumbTrade[] trades = {
                    new BithumbTrade("uuid-1", "bid", "limit", "50000000",
            ...
            assertThat(result.trades().get(0).fee()).isEqualByComparingTo(BigDecimal.ZERO);
        }
    }
    ...
    class GetMarkets {
        ...
        void filtersKrwMarkets() {
            stubRestClientChain();
            BithumbMarket[] markets = {
                    new BithumbMarket("KRW-BTC", "비트코인", "Bitcoin", "NONE"),
                    new BithumbMarket("KRW-ETH", "이더리움", "Ethereum", "NONE"),
                    new BithumbMarket("BTC-ETH", "이더리움", "Ethereum", "NONE"),
                    new BithumbMarket("USDT-BTC", "비트코인", "Bitcoin", "NONE")
            };
            given(responseSpec.body(BithumbMarket[].class)).willReturn(markets);
        ...
        void nullResponse() {
            stubRestClientChain();
            given(responseSpec.body(BithumbMarket[].class)).willReturn(null);
    ...
    class Retry {
        ...
        void retryOn429ThenSuccess() {
            stubRestClientChain();
            given(responseSpec.body(BithumbMarket[].class))
                    .willThrow(HttpClientErrorException.create(
                            HttpStatusCode.valueOf(429), "Too Many Requests", null, null, null))
                    .willReturn(new BithumbMarket[]{
                            new BithumbMarket("KRW-BTC", "비트코인", "Bitcoin", "NONE")
                    });
        ...
        void noRetryOn401() {
            stubRestClientChain();
            given(responseSpec.body(BithumbAccount[].class))
                    .willThrow(HttpClientErrorException.create(
                            HttpStatusCode.valueOf(401), "Unauthorized", null, null, null));
            ...
            then(responseSpec).should(times(1)).body(BithumbAccount[].class);
        }
    }
}
