...
class UpbitCollectorTest {

    @Mock private UpbitConnector connector;
    ...
    void getExchange() {
        assertThat(collector.getExchange()).isEqualTo(Exchange.UPBIT);
    }
    ...
    class CollectSymbol {
        ...
        void singlePage_success() {
            ...
            given(syncCursorRepository.findByUserIdAndExchangeAndSymbol(USER_ID, Exchange.UPBIT, "BTC"))
        ...
        void multiplePages() {
            ...
            TradePageResult page1 = new TradePageResult(List.of(trade1), true, "2024-01-15T10:00:00");
            ...
            given(connector.getTrades(apiKey, "BTC", "2024-01-15T10:00:00", 100)).willReturn(page2);
            given(tradeRepository.insertIgnore(anyLong(), anyString(), anyString(), anyString(),
                    anyString(), any(), any(), any(), anyString(), any(), anyString())).willReturn(1);
            given(syncCursorRepository.findByUserIdAndExchangeAndSymbol(USER_ID, Exchange.UPBIT, "BTC"))
                    .willReturn(Optional.empty());
        ...
        @Test
        @DisplayName("증분 수집 시 fromCursor부터 조회한다")
        void incrementalCollection() {
            ...
            given(connector.getTrades(apiKey, "ETH", fromCursor, 100)).willReturn(emptyPage);

            SymbolCollectionResult result = collector.collectSymbol(USER_ID, apiKey, "ETH", fromCursor);

            assertThat(result.success()).isTrue();
            assertThat(result.newTradesCount()).isEqualTo(0);
            then(connector).should().getTrades(apiKey, "ETH", fromCursor, 100);
        }
    ...
    class SyncBalances {
        ...
        void createsNewAsset() {
            ...
            given(assetRepository.findByUserIdAndExchangeAndCoin(USER_ID, Exchange.UPBIT, "BTC"))
                    .willReturn(Optional.empty());
        ...
        void updatesExistingAsset() {
            ...
            given(connector.getBalances(apiKey)).willReturn(balances);
            given(assetRepository.findByUserIdAndExchangeAndCoin(USER_ID, Exchange.UPBIT, "ETH"))
        ...
        void syncsMultipleAssets() {
            ...
            given(connector.getBalances(apiKey)).willReturn(balances);
            given(assetRepository.findByUserIdAndExchangeAndCoin(eq(USER_ID), eq(Exchange.UPBIT), anyString()))
                    .willReturn(Optional.empty());
